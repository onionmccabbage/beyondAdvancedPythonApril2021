...
class TempGetter(Thread):
    ...
    def run(self):
        ...
        # retrieve the map using lat lon values
        self.lat = data['coord']['lat']
        self.lon = data['coord']['lon']
        mapUrlTemplate = 'https://www.google.co.uk/maps/place/{},{}'
        mapResponse = urlopen(mapUrlTemplate.format(self.lat,self.lon))
        mapHtml = mapResponse.read()
        # write this retrieved map to a byte file (one for each city)
        file = open('c:/pythonBA/reviewDay2/map{}.html'.format(self.city), 'wb')
        file.write(mapHtml)
        file.close()
